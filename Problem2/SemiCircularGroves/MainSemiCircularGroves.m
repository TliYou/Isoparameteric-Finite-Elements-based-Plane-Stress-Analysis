...
for lp=1:length(ratio)
    ...
    nodes_lower = findNodes(meshAll(lp), 'box', [0 L/2], [-0.1 0.1]);   %nodes on right side
    nodes_left = findNodes(meshAll(lp), 'box', [0 0.1], [0 D/2-r]);       %nodes on left side
    nodes_right = findNodes(meshAll(lp),'box',[L/2-0.1,L/2+0.1],[0;D/2]);  %nodes on right side
    
%     fprintf('rho/r = %.2f', ratio(lp))
%     nodes_lower
%     nodes_left
%     nodes_right
